            ...
            <table class="w-full text-sm text-left text-gray-500">
                ...
                <tbody>
                    ...
                    <tr class="bg-white border-b">
                        ...
                        <td class="px-6 py-4">
                            <a href="{{ asset('pdfs/forecast/'.$controll->pdf_fortcast) }}" 
                               target="_blank"
                        ...
                        <td class="px-6 py-4">
                            <button onclick="deleteForecast('{{ $controll->id }}')"
...
<div id="createModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden overflow-y-auto h-full w-full">
    <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
        <div class="mt-3">
            <h3 class="text-lg leading-6 font-medium text-gray-900">Upload Forecast</h3>
            <form id="createForm" class="mt-4">
                ...
                <div class="mb-4">
                    ...
                    <input type="file" name="pdf_fortcast" accept=".pdf" required
                           class="shadow border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                </div>
...
document.getElementById('createForm').addEventListener('submit', async function(e) {
    ...
    try {
        const response = await fetch('/forecast-supplier', {
            method: 'POST',
            headers: {
...
async function deleteForecast(id) {
    if (confirm('Are you sure you want to delete this forecast?')) {
        try {
            const response = await fetch(`/forecast-supplier/${id}`, {
                method: 'DELETE',
                headers: {
                    'X-CSRF-TOKEN': document.querySelector('input[name="_token"]').value
                }
